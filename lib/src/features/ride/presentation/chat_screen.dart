...
class _ChatScreenState extends ConsumerState<ChatScreen> {
  ...
  void _send([String? preset]) {
    ...
    _ctrl.clear();
    ref.read(rideFlowProvider.notifier).sendMessage(text);
    _scrollToBottom();
  }
  ...
  Widget build(BuildContext context) {
    ...
    ref.listen<List<ChatMessage>>(
      rideFlowProvider.select((s) => s.chatMessages),
      ...
      body: Column(
        children: [
          _Header(
            name: driverName,
            subtitle: driverSub,
            onBack: () => backOr(context, '/tracking'),
          ),
          Expanded(
...
class _QuickReplies extends StatelessWidget {
  ...
  static const _presets = ["I'm outside", 'Which car?', '2 mins', 'Thanks!'];
