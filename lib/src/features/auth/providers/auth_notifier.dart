...
class AuthNotifier extends StateNotifier<AuthState> {
  ...
  /// Refreshes the cached profile from `GET /me` — picks up fields (e.g.
  /// emergency contact) that a restored session may not carry yet.
  Future<void> loadProfile() async {
    try {
      ...
      // Best-effort refresh — screens fall back to whatever's cached.
    }
  }
  ...
  Future<bool> uploadProfilePicture(File file) async {
    state = state.copyWith(isLoading: true, clearError: true);
    try {
      final profile = await _service.uploadProfilePicture(file);
      state = state.copyWith(isLoading: false, hasProfilePicture: profile.hasProfilePicture);
      await _persistSession();
      return true;
    } catch (e) {
      state = state.copyWith(isLoading: false, error: friendlyError(e));
  ...
  Future<void> _applyAuth(AuthResult result) async {
    ...
    state = result.toAuthState().copyWith(isLoading: false, expiresAt: expiresAt);
    await _persistSession();
  }
